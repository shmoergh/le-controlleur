...
SequencerEngine::SequencerEngine(Brain& brain) :
	brain_(brain),
...
void SequencerEngine::update() {
	...
	const bool pulse_in_high = brain_.inputs.pulse_read();
	...
	if (gate_active_ && now_us >= gate_off_time_us_) {
		brain_.outputs.pulse_set(false);
		gate_active_ = false;
	}
...
void SequencerEngine::scan_pots_snapshot() {
	...
	for (uint8_t i = 0; i < NUM_POTS; ++i) {
		pot_raw_values_[i] = static_cast<uint8_t>(brain_.pots.get_buffered(i));
	}
}
...
void SequencerEngine::on_mode_enter() {
	...
	brain_.leds.set_mode(LedMode::kPwm);
	reset_transport();
	root_edit_armed_ = false;
	shift_active_ = false;
...
void SequencerEngine::init_io() {
	...
	brain_.outputs.set_output_range(AudioCvOutChannel::kChannelA, AudioCvOutRange::kRange0To10V);
	brain_.outputs.set_output_range(AudioCvOutChannel::kChannelB, AudioCvOutRange::kRange0To10V);
...
void SequencerEngine::init_pot_functions() {
	...
	brain_.pot_multi.register_function(bpm_cfg);
	...
	brain_.pot_multi.register_function(swing_cfg);
	...
	brain_.pot_multi.register_function(range_cfg);
	...
	brain_.pot_multi.register_function(quant_cfg);
	...
	brain_.pot_multi.register_function(randomness_cfg);
	...
	brain_.pot_multi.register_function(length_cfg);
}
...
void SequencerEngine::set_active_pot_functions(uint8_t pot_0_function, uint8_t pot_1_function, uint8_t pot_2_function) {
	...
	brain_.pot_multi.set_active_functions(active_functions, NUM_POTS);
	brain_.pot_multi.update_buffered(brain_.pots, false);
}
...
void SequencerEngine::update_bpm_from_pot(bool force_apply) {
	if (!force_apply && !brain_.pot_multi.get_changed(POT_FUNCTION_ID_BPM)) {
		return;
	}
	const uint8_t pot_value = brain_.pot_multi.get_value(POT_FUNCTION_ID_BPM);
...
void SequencerEngine::update_swing_from_pot1(bool force_apply) {
	...
	if (!force_apply && !brain_.pot_multi.get_changed(POT_FUNCTION_ID_SWING)) {
		return;
	}
	const uint8_t pot_value = brain_.pot_multi.get_value(POT_FUNCTION_ID_SWING);
...
void SequencerEngine::update_range_or_quantization_from_pot2(bool force_apply) {
	...
	if (!force_apply && !brain_.pot_multi.get_changed(function_id)) {
		return;
	}
	const uint8_t pot_value = brain_.pot_multi.get_value(function_id);
...
void SequencerEngine::update_pot_led_overlay(uint64_t now_us) {
	...
	pot_led_overlay_active_ = false;
	brain_.leds.set_from_mask(gate_history_mask());
}
...
void SequencerEngine::show_active_pot_overlay(uint8_t pot_index) {
	...
	brain_.leds.set_from_mask(mask);
}
...
void SequencerEngine::show_transpose_overlay() {
	...
	for (uint8_t i = 0; i < NO_OF_LEDS; ++i) {
		if (i < full_count) {
			brain_.leds.set_brightness(i, 255u);
		} else {
			brain_.leds.off(i);
		}
	}
	...
	if (dim_index >= 0 && dim_index < static_cast<int8_t>(NO_OF_LEDS)) {
		brain_.leds.set_brightness(static_cast<uint8_t>(dim_index), POT_LED_SOFT_BRIGHTNESS);
	}
}
...
void SequencerEngine::write_pitch_voltage(AudioCvOutChannel channel, float voltage) {
	...
	if (calibrated_output_enabled_) {
		brain_.outputs.set_voltage_calibrated_millivolts(channel, millivolts);
		return;
	}
	brain_.outputs.set_voltage_millivolts(channel, millivolts);
}
...
void SequencerEngine::tick(uint64_t now_us) {
	...
	if ((tick_counter_ % STEPS_PER_QUARTER_NOTE) == 0) {
		brain_.leds.button_blink_duration(BUTTON_LED_BLINK_MS, BUTTON_LED_BLINK_INTERVAL_MS);
	}
...
void SequencerEngine::reset_transport() {
	...
	root_edit_octave_pot_reference_raw_ = pot_raw_values_[POT_INDEX_RANGE_OR_QUANTIZATION];
	root_edit_note_pot_reference_raw_ = pot_raw_values_[POT_INDEX_RANDOMNESS_OR_LENGTH];
	last_pulse_in_high_ = brain_.inputs.pulse_read();
	last_external_tick_us_ = 0;
	external_interval_us_ = tick_interval_us_;
	last_midi_clock_tick_us_ = 0;
	last_midi_event_us_ = 0;
